...
class AccountMove(models.Model):
    ...
    def _l10n_ke_oscu_json_from_move(self):
        ...
        tax_codes = {item['code']: item['tax_rate'] for item in self.env['l10n_ke_etims_vscu.code'].search([('code_type', '=', '04')])}
    ...
    def _l10n_ke_oscu_send_customer_invoice(self):
        ...
        error, data, _date = company._l10n_ke_call_etims('trnsSales/saveSales', content)
        if not error:
            self.write({
    ...
    def action_l10n_ke_oscu_confirm_vendor_bill(self):
        ...
        Vendor bills can be received from the VSCU or created locally. When confirming vendor bills
    ...
    def _l10n_ke_oscu_fetch_purchases(self, companies):
        ...
        for company in companies:
            ...
            if error:
                if error['code'] == '001':
                    _logger.warning("There are no new vendor bills on the VSCU for %s.", company.name)
                else:
                    _logger.error("Error retrieving purchases from the VSCU: %s: %s", error['code'], error['message'])
                continue
    ...
    def _cron_l10n_ke_oscu_fetch_purchases(self):
        ...
        companies = self.env['res.company'].search([('l10n_ke_oscu_serial_number', '!=', False)])
    ...
    def _l10n_ke_oscu_import_invoice(self, invoice, data, is_new):
        ...
        This method is passed as the EDI decoder in the case where the file is recognised as an VSCU
    ...
    def _get_name_invoice_report(self):
        ...
        if self.l10n_ke_oscu_invoice_number:
            return 'l10n_ke_etims_vscu.report_invoice_document'
