...
class ResCompany(models.Model):
    ...
    l10n_ke_server_mode = fields.Selection(
        ...
        help="""
            ...
            - Demo: Mocked data, does not require an initialized VSCU.
        """,
    )
    ...
    l10n_ke_oscu_user_agreement = fields.Boolean(
        string="Odoo VSCU user agreement",
        help="Agreement is required to use Odoo as an VSCU service provider.",
        tracking=True,
    )
    l10n_ke_oscu_is_active = fields.Boolean(
        string="VSCU is Active",
    )
    ...
    def write(self, vals):
        ...
        if email_is_not_sent:
            for company in email_is_not_sent.filtered(lambda c: c.l10n_ke_oscu_user_agreement and c.l10n_ke_oscu_cmc_key):
                logging_message = f"""
                    Checkbox `Odoo VSCU user agreement` was set to True
                ...
                mail_body = Markup("""
                    <div style="margin: 0px; padding: 0px;">
                        <p style="margin: 0px; padding: 0px;">
                            Dear %s,
                            <br/>
                            This is a notification that you have agreed to Odoo's VSCU user agreement. %s can now
                            use VSCU flows in Odoo to declare your activities with the KRA.
                            <br/>
                ...
                mail_subject = f" {company.name} - Odoo VSCU User Agreement confirmation"
                mail_values = {
    ...
    def _l10n_ke_preinitialization_checks(self):
        if not self.l10n_ke_oscu_user_agreement:
            raise UserError(_("Please agree to the terms of use of Odoo as an VSCU service provider first. "))
        error_fields = []
        on_company = False
        if not self.vat:
        ...
        if not self.l10n_ke_oscu_serial_number:
            error_fields.append(_('VSCU Serial Number'))
        if not self.l10n_ke_api_url:
            error_fields.append(_('VSCU API URL'))
        if error_fields:
    ...
    def action_l10n_ke_oscu_initialize(self):
        ...
        url = urljoin(self._l10n_ke_oscu_get_base_url(), 'initializer/selectInitInfo')
        _logger.info("Calling VSCU initialization")
        try:
        ...
        if response_content['resultCd'] != '000':
            if response_content['resultCd'] == '901':
                raise ValidationError(_('The registration failed.  Maybe you did not do the necessary steps with '
                                        'the KRA or the device has been registered before elsewhere'))
    ...
    def action_l10n_ke_get_items(self):
        ...
        """
        last_request_date = self.env['ir.config_parameter'].get_param('l10n_ke_etims_vscu.last_fetch_items_request_date', '20180101000000')
    ...
    def _l10n_ke_oscu_get_base_url(self):
        return self.l10n_ke_api_url

    def _l10n_ke_call_etims(self, urlext, content):
        """ Make a request to the VSCU
        ...
        _logger.info("Calling endpoint: %s", urlext)
        _logger.info(json.dumps(content, indent=4))
        _logger.info(self.vat)
        _logger.info(self.l10n_ke_api_url)
    ...
    def _l10n_ke_get_demo_response(self, urlext, content):
        ...
        module = 'l10n_ke_etims_vscu_stock' if urlext in stock_services else 'l10n_ke_etims_vscu'
    ...
    def _l10n_ke_find_for_cron(self, failed_action=''):
        ...
        if not company:
            _logger.warning("No VSCU initialized company could be found. %s", failed_action)
