...
class L10nKeVSCUCode(models.Model):
    _name = 'l10n_ke_etims_vscu.code'
    ...
    def _create_or_update_from_vals(self, create_vals):
        ...
        :param list[dict] create_vals: list of l10n_ke_etims_vscu.code creation vals.
        ...
        model_data_values = [
            {
                ...
                'model': 'l10n_ke_etims_vscu.code',
                'module': 'l10n_ke_etims_vscu',
                'res_id': code.id,
                'noupdate': True,
            }
            for code in created
    ...
    def _cron_get_codes_from_device(self):
        ...
        error, data, _date = company._l10n_ke_call_etims('code/selectCodes', {'lastReqDt': last_request_date})
        if error:
            if error['code'] == '001':
                _logger.info("No new KRA standard codes fetched from the VSCU.")
                return
        ...
        _logger.info("Fetched KRA standard codes from the VSCU, created %i and updated %i.", len(created), len(updated))
