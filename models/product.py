...
from odoo.addons.l10n_ke_etims_vscu.models.account_move import format_etims_datetime
...
class ProductProduct(models.Model):
    ...
    @api.depends('type')
    def _compute_l10n_ke_packaging_unit_id(self):
        service_packaging = self.env.ref('l10n_ke_etims_vscu.packaging_type_ou', raise_if_not_found=False)
    ...
    def _l10n_ke_get_validation_messages(self, for_invoice=False):
        ...
        products_incorrect_taxes = self.filtered(
            lambda p: len(p.taxes_id.filtered(lambda t: t.l10n_ke_tax_type_id and t.company_id in [self.env.company.id])) > 1
        )
    ...
    def _l10n_ke_get_tax_type(self):
        ...
        self.ensure_one()
        return self.taxes_id.l10n_ke_tax_type_id
    ...
    def _l10n_ke_action_open_products(self, title=None):
        ...
        res = {
            ...
            'view_mode': 'tree',
            'views': [(self.env.ref('l10n_ke_etims_vscu.l10n_ke_kra_product_tree').id, 'tree'), (False, 'form')],
...
class ProductCode(models.Model):
    ...
    l10n_ke_tax_type_id = fields.Many2one('l10n_ke_etims_vscu.code')

    def _cron_l10n_ke_oscu_get_codes_from_device(self):
        """ Automatically fetch and create UNSPSC codes from the VSCU if they don't already exist """
        ...
        tax_codes = {
            ...
            for tax_code in self.env['l10n_ke_etims_vscu.code'].search_read([('code_type', '=', '04')], ['code'])
        }
        last_request_date = self.env['ir.config_parameter'].get_param('l10n_ke_oscu.last_unspsc_code_request_date', '20180101000000')
        error, data, _date = company._l10n_ke_call_etims('itemClass/selectItemsClass', {'lastReqDt': last_request_date})
        if error:
            if error.get('code') == '001':
                _logger.info("No new UNSPSC codes fetched from the VSCU.")
                return
            raise UserError(f"[{error['code']}] {error['message']}")
        ...
        _logger.info("%i UNSPSC codes fetched from the VSCU, %i UNSPSC codes created", len(cls_list), len(new_codes))
